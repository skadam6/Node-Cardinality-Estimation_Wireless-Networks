...
for i=1:Hmax
       ...
       if((temp(i,1)>=2)|| (temp(i,2)>=2) || ((temp(i,1)== 1)&&(temp(i,2)== 1))) %Specific to 2 types of nodes case.
            K=K+1;          
       end
       if ((temp(i,1)>=2)) % 3rd Phaase where we cheack for second type of device
            R=R+1;
       end
end
...
x=0.02:0.02:q_max; %case of N variation
...
savefig('Slots_Required_2');
save 2dev.txt Exp_time_slots -ascii %To save the values, which can be used in 4dev, 6dev etc.
