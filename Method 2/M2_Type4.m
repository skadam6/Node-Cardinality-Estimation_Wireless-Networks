...
b = 6; % slot width
ncc = 2; % Number of case  which require additional slots in next phase
...
for q=0.02:0.02:0.2
    ...
    for iter =1:Max_iter
        ...
        for i=1:Hmax
            ...
            switch 1
               case ( (temp(i,1) == 0) && (temp(i,2)==1) && (temp(i,3)>=1) && (temp(i,4)==0)) % (alpha,C) case
                    R1=R1+1;
               case ( (temp(i,1) == 1) && (temp(i,2)==0) && (temp(i,3)>=2) && (temp(i,4)==0)) % (alpha,C) case
                    R1=R1+1;
               case ( (temp(i,3) == 0) && (temp(i,4)==1) && (temp(i,1)>=1) && (temp(i,2)==0)) % (C, beta) case
                    R1=R1+1;
               case ( (temp(i,3) == 1) && (temp(i,4)==0) && (temp(i,1)>=2) && (temp(i,2)==0)) % (C, beta) case
                    R1=R1+1;
               ...
               case ((temp(i,2) == 1) && (temp(i,4)==1)) % (C,C) case
                    R2=R2+1;
               case ( (temp(i,1) >= 1) && (temp(i,2)==1) && (temp(i,3)>=1) && (temp(i,4)==0)) % (C,C) case
                    R2=R2+1;
               case ( (temp(i,1) >= 1) && (temp(i,2)==0) && (temp(i,3)>=1) && (temp(i,4)==1)) % (C,C) case
                    R2=R2+1;
               case ( (temp(i,1) >= 2) && (temp(i,2)==0) && (temp(i,3)>=2) && (temp(i,4)==0)) % (C,C) case
                    R2=R2+1;
            end
           
        end
        
       Prop_Method(iter) = 2*Hmax+ (len*Hmax)/b + R1+ R2*(2 + (Two_Dev_Result(r)/Hmax)); %2 slots for Type 2 and Type 4 , /Hmax bcoz in prev case, results calculated for Hmax blocks.      
        display(iter)
    end
    display(q)
...
savefig('Slots_Required_4');
save 4dev.txt Exp_time_slots -ascii %To save the values, which can be used in 6dev, 8dev etc.
